...
st.write("시가총액 상위 300개 종목을 기준으로 시장의 과열 및 공포 구간을 분석합니다.")
...
if st.button(f'{market_type} 상위 {num_stocks}개 데이터 분석 시작'):
    with st.spinner(f'{market_type}의 데이터를 정밀 분석 중입니다... (약 1~2분 소요)'):
        # 1. 종목 리스트 확보 (선택한 시장의 상위 N개)
        ...
        for i, (ticker, name) in enumerate(zip(tickers, names)):
            try:
                # 최근 2년치를 가져와서 50일선 계산 (데이터 누락 방지)
                df = fdr.DataReader(ticker)
                if len(df) > 60:
                    ma50 = df['Close'].rolling(window=50).mean()
                    # 현재가가 50일선보다 낮으면 1(하회), 높으면 0
        ...
        # 3. 날짜별 하회 비율 계산 (%)
        # 전체 종목 수 대비 하회 종목 비율 계산
        ratio_series = history_df.mean(axis=1) * 100
        ratio_series = ratio_series.tail(252) # 최근 1년치만 추출

        # 4. 그래프 시각화
        fig = go.Figure()
        
        # 기준선 추가 (80% 과매도, 20% 과매수)
